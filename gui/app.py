...
class SIDE_App(tk.Tk, TeamsCanvasMixin):
    ...
    def __init__(self):
        ...
        self._manager: object = None        # ai.manager.Manager
    ...
    def _run_profiler(self) -> None:
        ...
        def _run():
            ...
            def _done():
                if btn:
                    if result.ok:
                        btn.config(text="⏱ Profile", fg=P["green"])
                    else:
                        btn.config(text="⏱ Profile", fg=P["red"])
                    self.after(3000, lambda: btn.config(fg=P["t2"]))
                summary = result.summary()
                self._ai_append(f"\n{summary}\n", "dim")
                if result.ok:
                    self._ai_append(
                        f"\nMetrics written to {result.metrics_path}\n"
                        "Node cards will update within 2s.\n", "tool")
                    # Restart MetricsWatcher to pick up new data
                    self._start_metrics_watcher(root)
                else:
                    self._ai_append(f"\nError: {result.error}\n", "error")
            self.after(0, _done)
    ...
    def _ai_send(self):
        ...
        if bm:
            self._manager.bake(task=prompt, minutes=int(bm.group(1)))
    ...
    def _build_topbar(self):
        ...
        # Profile button
        self._profile_btn = tk.Label(tb, text="⏱ Profile", bg=P["bg3"], fg=P["t2"],
                                      font=self._mono_xs, padx=7, pady=3,
                                      cursor="hand2",
                                      highlightbackground=P["line2"], highlightthickness=1)
