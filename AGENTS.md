...
- `test/test_suite.py` — the whole test suite, stdlib `unittest`, currently 112 tests / 3
  failing (see Round 1).
- `web-infra.json` — hand-authored n3xu5 infrastructure graph, served via `/api/infra`.
  Currently untracked; Round 1 decides whether to commit it.
